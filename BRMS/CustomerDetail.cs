...
namespace BRMS
{
    public partial class CustomerDetail : Form
    {
        ...
        {
            panelOrderDataGrid.Controls.Add(custOrder.Dgv);
            custOrder.Dgv.Dock = DockStyle.Fill;
            panelSaleDataGrid.Controls.Add(custSale.Dgv);
            custSale.Dgv.Dock = DockStyle.Fill;
            pnlLogDataGrid.Controls.Add(custLog.Dgv);
            custLog.Dgv.Dock = DockStyle.Fill;

            custOrder.Dgv.Columns.Add("custOrderCode", "주문코드");
            custOrder.Dgv.Columns.Add("custOrderStatus", "상태");
            custOrder.Dgv.Columns.Add("custOrderDate", "주문일");
            custOrder.Dgv.Columns.Add("custOrderAmontKrw", "주문액(￦)");
            custOrder.Dgv.Columns.Add("custOrderAmontUsd", "주문액(＄)");
            ...
            custOrder.Dgv.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            custOrder.ApplyDefaultColumnSettings();

            custSale.Dgv.Columns.Add("saleCode", "판매코드");
            custSale.Dgv.Columns.Add("saleType", "유형");
            custSale.Dgv.Columns.Add("saleDate", "판매일");
            custSale.Dgv.Columns.Add("saleStaff", "담당자");
            custSale.Dgv.Columns.Add("saleAmountKrw", "판매액(￦)");
            custSale.Dgv.Columns.Add("saleAmountUsd", "판매액(＄)");
            ...
            custSale.Dgv.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            custSale.ApplyDefaultColumnSettings();

            custLog.Dgv.Columns.Add("logType", "작업내역");
            custLog.Dgv.Columns.Add("logBefore", "변경전");
            custLog.Dgv.Columns.Add("logAfter", "변경후");
            custLog.Dgv.Columns.Add("logEmp", "작업자");
            custLog.Dgv.Columns.Add("logDate", "변경일");
            custLog.Dgv.ReadOnly = true;
            custLog.ApplyDefaultColumnSettings();
            custLog.Dgv.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
        ...
        private void LoadOrderData()
        {
            ...
            DataTable dataTable = new DataTable();
            dbconn.SqlDataAdapterQuery(query, dataTable);
            custOrder.Dgv.Rows.Clear();
            foreach (DataRow dataRow in dataTable.Rows)
            {
                var rowIndex = custOrder.Dgv.Rows.Add();
                var row = custOrder.Dgv.Rows[rowIndex];
                row.Cells["No"].Value = custOrder.Dgv.RowCount;
        ...
        {
            int orderCode = custOrder.ConvertToInt(custOrder.Dgv.CurrentRow.Cells["custOrderCode"].Value);
        ...
        private void LoadSaleData()
        {
            ...
            DataTable dataTable = new DataTable();
            dbconn.SqlDataAdapterQuery(query, dataTable);
            custSale.Dgv.Rows.Clear();
            foreach(DataRow dataRow in dataTable.Rows)
            {
                var rowIndex = custSale.Dgv.Rows.Add();
                var row = custSale.Dgv.Rows[rowIndex];
                row.Cells["No"].Value = custSale.Dgv.RowCount ;
        ...
        private void LoadCustLog(DataTable dataTable)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                int addRow = custLog.Dgv.Rows.Add();
                // 로그 데이터 설정
                ...
                // 해당 셀에 값 설정
                custLog.Dgv.Rows[addRow].Cells["No"].Value = addRow + 1;
                custLog.Dgv.Rows[addRow].Cells["logType"].Value = logType; 
                custLog.Dgv.Rows[addRow].Cells["logBefore"].Value = before;
                custLog.Dgv.Rows[addRow].Cells["logAfter"].Value = after;
                custLog.Dgv.Rows[addRow].Cells["logEmp"].Value = empName + $"({empCode})";
                custLog.Dgv.Rows[addRow].Cells["logDate"].Value = logDate;
            }
        }
        /// <summary>
        /// 새 고객 등록 쿼리
