...
namespace BRMS
{
    public partial class PaymentDetail : Form
    {
        ...
        /// <summary>
        /// 수정권한이 없을 경우 수정 및 삭제 관련 버튼 비활성화
        /// </summary>
        private void ModifyPermission()
        {
            if (!cUserSession.HasPermission(222))
            {
                ...
                bntAccountSet.Enabled = false;
                bntPaymentDate.Enabled = false;
        ...
        private void ModifyPayment(SqlConnection connection, SqlTransaction transaction)
        {
        ...
        private void bntSave_Click(object sender, EventArgs e)
        {
            using (SqlConnection connection = dbconn.Opensql())
            {
                ...
                try
                {
                    ...
                    {
                        ModifyPayment(connection, transaction);
