...
namespace BRMS
{
    public partial class SalesReport : Form
    {
        ...
        public SalesReport()
        {
            ...
            cmBoxSearchTypeSet();
            lblCategory.Text = "전체";
            panelDatagrid.Controls.Add(dgrReport.Dgv);
            dgrReport.Dgv.Dock = DockStyle.Fill;
            ...
            dgrReport.Dgv.ColumnHeaderMouseClick += dgrReport_ColumnHeaderMouseClick;
        ...
        private void GridForm(string[] name, string[] text)
        {
            ...
            dgrReport.Dgv.Columns.Clear();
            for (int i = 0; i < name.Length; i++)
            {
                dgrReport.Dgv.Columns.Add(name[i], text[i]);
            }
            ...
            dgrReport.Dgv.ReadOnly = true;
            dgrReport.Dgv.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            switch (cmBoxSearchType.SelectedIndex)
            {
                case 0:
                    dgrReport.Dgv.Columns["pdtCode"].Visible = false;
                    dgrReport.Dgv.Columns["pdtNameKr"].DefaultCellStyle.BackColor = Color.FromArgb(234, 238, 244);
                    dgrReport.Dgv.Columns["pdtNameKr"].DefaultCellStyle.SelectionBackColor = Color.FromArgb(234, 238, 244);
                    dgrReport.Dgv.Columns["pdtNameKr"].DefaultCellStyle.Padding = new Padding(1);
                    break;
                case 1:
                    dgrReport.Dgv.Columns["catTop"].Visible = false;
                    dgrReport.Dgv.Columns["catMid"].Visible = false;
                    dgrReport.Dgv.Columns["catBot"].Visible = false;
                    break;
            }
            if(chkDayli.Checked)
            {
                foreach (DataGridViewColumn column in dgrReport.Dgv.Columns)
                {
       ...
       /// <param name="e"></param>
        ...
        private void SearchQuerySetting()
        {
            ...
            //Aggregate(saleTable);
            FillGrid(saleTable);
            cLog.InsertEmpAccessLogNotConnect("@salesReportSearch", accessedEmp, 0);
        }
        private void FillGrid(DataTable dataTable)
        {
            ...
            var dayliSum = new Dictionary<string, (int dayliRevenue, decimal dayliBprice, decimal dayliProfit, decimal dayliMargin, decimal dayRatio, int dayliDc, int dayliSale, int dayliReturn, int dayliTax)>();
            ...
            if (chkDayli.Checked)
            {
                ...
                foreach (var saleGroup in dataTable.AsEnumerable().GroupBy(row => row.Field<string>("sale_dt")))
                {
                    ...
                    int sumRevenue = saleGroup.Sum(r => Convert.ToInt32(r["revenue"]));
                    int sumCost = saleGroup.Sum(r => Convert.ToInt32(r["bprice"]));
                    int sumDiscount = saleGroup.Sum(r => Convert.ToInt32(r["discount"]));
                    int sumTax = saleGroup.Sum(r => Convert.ToInt32(r["tax"]));
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                switch (searchType)
                {
                    case 0: // pdtCode로 중복 확인
                        foreach (DataGridViewRow searchRow in dgrReport.Dgv.Rows)
                        {
                ...
                if (duplicate == false)
                {
                    rowIndex = dgrReport.Dgv.RowCount;
                    dgrReport.Dgv.Rows.Add();
                    //일자별 체크시 현재의 날짜와 마지막 데이터 그리드의 매출일자가 다를 경우 일자별 합계를 데이터 그리드에 추가 
                    if(chkDayli.Checked)
                    {
                        saleDate = row["sale_dt"].ToString();
                        
                        if (rowIndex != 0 && saleDate != dgrReport.Dgv.Rows[rowIndex - 1].Cells["saleDate"].Value.ToString())
                        {
                    ...
                    switch (searchType)
                    {
                        case 0:
                            ...
                            dgrReport.Dgv.Rows[rowIndex].Cells["pdtCode"].Value = row["pdt_code"];
                            dgrReport.Dgv.Rows[rowIndex].Cells["pdtNameKr"].Value = readrow[0].ToString().Trim();
                            dgrReport.Dgv.Rows[rowIndex].Cells["pdtNumber"].Value = readrow[1].ToString().Trim();
                            break;
                        ...
                        case 1:
                            ...
                            dgrReport.Dgv.Rows[rowIndex].Cells["catTop"].Value = row["cattop"];
                            dgrReport.Dgv.Rows[rowIndex].Cells["catMid"].Value = row["catmid"];
                            dgrReport.Dgv.Rows[rowIndex].Cells["catbot"].Value = row["catbot"];
                            dgrReport.Dgv.Rows[rowIndex].Cells["categoryName"].Value = resultObj.ToString();
                            break;
                        ...
                        case 3:
                            dgrReport.Dgv.Rows[rowIndex].Cells["saleDate"].Value = row["sale_dt"].ToString();
                            break;
                    }
                    dgrReport.Dgv.Rows[rowIndex].Cells["revenue"].Value = revenue == 0 ? 0 : revenue;
                    dgrReport.Dgv.Rows[rowIndex].Cells["cost"].Value = bprice == 0 ? 0 : bprice;
                    dgrReport.Dgv.Rows[rowIndex].Cells["profit"].Value = profit == 0 ? 0 : profit;
                    dgrReport.Dgv.Rows[rowIndex].Cells["margin"].Value = margin == 0 ? 0 : margin;
                    dgrReport.Dgv.Rows[rowIndex].Cells["ratio"].Value = revenue ==0 ? 0 : Convert.ToDecimal(revenue) / Convert.ToDecimal(totalRevenue) * 100;
                    dgrReport.Dgv.Rows[rowIndex].Cells["sale"].Value = sale == 0 ? 0 : sale;
                    dgrReport.Dgv.Rows[rowIndex].Cells["return"].Value = returns == 0 ? 0 : returns;
                    dgrReport.Dgv.Rows[rowIndex].Cells["dc"].Value = dc == 0 ? 0 : dc;
                    dgrReport.Dgv.Rows[rowIndex].Cells["tax"].Value = tax == 0 ? 0 : tax;
                    if(chkDayli.Checked && searchType != 3)
                    {
                       
                        dgrReport.Dgv.Rows[rowIndex].Cells["saleDate"].Value = row["sale_dt"];
                    }
                ...
                else
                {
                    ...
                    var existingRow = dgrReport.Dgv.Rows[rowIndex];
            ...
            {
                if (dgrReport.Dgv.RowCount > 0)
                {
                    int dayliRow = dgrReport.Dgv.Rows.Add();
                    AddDaliSaleDate(dayliRow, dayliSum);
                }

            }
            int newRow = dgrReport.Dgv.Rows.Add();
            switch(searchType)
            {
                case 0: //제품별
                    dgrReport.Dgv.Rows[newRow].Cells["pdtNameKr"].Value = "합계";
                    break;
                case 1: //분류별
                    dgrReport.Dgv.Rows[newRow].Cells["categoryName"].Value = "합계";
                    break;
                case 2: //공급사별
                    dgrReport.Dgv.Rows[newRow].Cells["supplier"].Value = "합계";
                    break;
                case 3: //일자별
                    dgrReport.Dgv.Rows[newRow].Cells["saleDate"].Value = "합계";
                    break;
            }
            decimal totalRatio = totalRevenue == 0 ? 0 : 100;
            dgrReport.Dgv.Rows[newRow].Cells["revenue"].Value = totalRevenue;
            dgrReport.Dgv.Rows[newRow].Cells["cost"].Value = totalBprice;
            dgrReport.Dgv.Rows[newRow].Cells["profit"].Value = totalProfit;
            dgrReport.Dgv.Rows[newRow].Cells["margin"].Value = totalMargin;
            dgrReport.Dgv.Rows[newRow].Cells["ratio"].Value = totalRatio;
            dgrReport.Dgv.Rows[newRow].Cells["sale"].Value = totalSale;
            dgrReport.Dgv.Rows[newRow].Cells["return"].Value = totalReturn;
            dgrReport.Dgv.Rows[newRow].Cells["dc"].Value = totalDC;
            dgrReport.Dgv.Rows[newRow].Cells["tax"].Value = totalTax;
            dgrReport.Dgv.Rows[newRow].DefaultCellStyle.BackColor = Color.LightGray;
            dgrReport.Dgv.Rows[newRow].DefaultCellStyle.SelectionBackColor = Color.LightGray;
        }
        /// <summary>
        /// 일자별 로우 데이터 생성 메소드
        /// 일자별 체크시 각 항목의 일자별 데이터 생성
        /// </summary>
        /// <param name="rowIndex"></param>
        /// <param name="dayliSum"></param>
        private void AddDaliSaleDate(int rowIndex, Dictionary<string, (int dayliRevenue, decimal dayliBprice, decimal dayliProfit, decimal dayliMargin, decimal dayRatio, int dayliDc, int dayliSale, int dayliReturn, int dayliTax)> dayliSum)
        {
  
            string keyValue = dgrReport.Dgv.Rows[rowIndex - 1].Cells["saleDate"].Value.ToString();
            if (dayliSum.TryGetValue(keyValue, out var dayliValue))
            {
                dgrReport.Dgv.Rows[rowIndex].Cells["revenue"].Value = dayliValue.dayliRevenue;
                dgrReport.Dgv.Rows[rowIndex].Cells["cost"].Value = dayliValue.dayliBprice;
                dgrReport.Dgv.Rows[rowIndex].Cells["profit"].Value = dayliValue.dayliProfit;
                dgrReport.Dgv.Rows[rowIndex].Cells["margin"].Value = dayliValue.dayliMargin;
                dgrReport.Dgv.Rows[rowIndex].Cells["ratio"].Value = dayliValue.dayRatio;
                dgrReport.Dgv.Rows[rowIndex].Cells["sale"].Value = dayliValue.dayliSale;
                dgrReport.Dgv.Rows[rowIndex].Cells["return"].Value = dayliValue.dayliReturn;
                dgrReport.Dgv.Rows[rowIndex].Cells["dc"].Value = dayliValue.dayliDc;
                dgrReport.Dgv.Rows[rowIndex].Cells["tax"].Value = dayliValue.dayliTax;
                dgrReport.Dgv.Rows[rowIndex].Cells["saleDate"].Value = keyValue;
            }

            switch (cmBoxSearchType.SelectedIndex)
            {
                case 0:
                    dgrReport.Dgv.Rows[rowIndex].Cells["pdtNameKr"].Value = keyValue;
                    break;
                case 1:
                    dgrReport.Dgv.Rows[rowIndex].Cells["categoryName"].Value = keyValue;
                    break;
                case 2:
                    dgrReport.Dgv.Rows[rowIndex].Cells["supplier"].Value = keyValue;
                    break;
            }
            dgrReport.Dgv.Rows[rowIndex].DefaultCellStyle.BackColor = cUIManager.Color.GreenGray;
            dgrReport.Dgv.Rows[rowIndex].DefaultCellStyle.SelectionBackColor = cUIManager.Color.GreenGray;
        }
