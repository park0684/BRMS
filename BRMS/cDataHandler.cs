...
namespace BRMS
{
    class cDataHandler
    {
        ...
        public static DateTime ConvertIntToDate(object input)
        {
            ...
            string month="";
            string day="";
            string date = input.ToString().Trim();
            if(date.Length.Equals(8))
            {
                year = date.Substring(0, 4);
                month = date.Substring(4, 2);
                day = date.Substring(6, 2);
            }
            else if(date.Length.Equals(6))
            {
                year = date.Substring(0, 2);
                month = date.Substring(2, 2);
                day = date.Substring(4, 0);
            }
        ...
        /// <summary>
        /// 이익율 계산 메소드
        /// 조회된 자료가 null 또는 0 경우 오류 발생 방지를 위해 사용
        /// </summary>
        /// <param name="sale"></param>
        /// <param name="cost"></param>
        /// <returns></returns>
        public static decimal ConvertProfitRate(object sale, object cost)
        {
            if(string.IsNullOrEmpty(sale.ToString())|| sale.ToString() == "0")
            {
                return 0;
            }
            if (string.IsNullOrEmpty(cost.ToString()) || cost.ToString() == "0")
            {
                return 100;
            }
